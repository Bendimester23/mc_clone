...
namespace world {
    ...
    void Chunk::Upload() {
        ...
        glBufferData(GL_ARRAY_BUFFER, this->m_Vertices.size() * sizeof(unsigned int), this->m_Vertices.data(),
                     GL_DYNAMIC_DRAW);

        glBindVertexArray(VAO);
        glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, indexVBO);
        glBufferData(GL_ELEMENT_ARRAY_BUFFER, this->m_Indices.size() * sizeof(unsigned short), this->m_Indices.data(),
                     GL_DYNAMIC_DRAW);
    ...
    ChunkCoord Chunk::GetPosition() const {
        return this->m_Pos;
    }
